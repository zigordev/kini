...
APP_ENV_FILE="$(mktemp /tmp/kini-app-env.XXXXXX)"
...
echo "[deploy] Starting kini app stack"
...
retry 30 2 run_compose --env-file "$APP_ENV_FILE" -f docker/compose.app.prod.yml exec -T kini_web sh -lc "wget -qO- http://kini-api:3000/health >/dev/null"

echo "[deploy] Health checking web container"
retry 30 2 run_compose --env-file "$APP_ENV_FILE" -f docker/compose.app.prod.yml exec -T kini_web sh -lc "wget -qO- http://localhost:3001/ >/dev/null"
